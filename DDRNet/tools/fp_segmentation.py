...
LastEditTime: 2024-05-22 16:06:06
...
def ridge_coherence(img, win_size=8, stride=8):
    ...
    coh = (F.avg_pool2d(torch.tensor(coh)[None, None].float(),
                        2 * win_size - 1,
                        stride=stride,
                        padding=win_size - 1).squeeze().numpy())
    return coh


def ridge_intensity(img, win_size=8, stride=8):
    coh = (F.avg_pool2d(torch.tensor(img)[None, None].float(),
                        2 * win_size - 1,
                        stride=stride,
                        padding=win_size - 1).squeeze().numpy())
    return coh
...
def segmentation_postprocessing(seg, kernel_size=5, stride=8, convex=False):
    ...
    seg = morphology.remove_small_holes(seg,
                                        area_threshold=15000 // (stride**2))
...
def segmentation_coherence(img,
                           win_size=16,
                           stride=8,
                           threshold=50,
                           convex=False,
    ...
    if stride != 1:
        coh = ndi.zoom(coh, (img.shape[0] * 1.0 / coh.shape[0],
                             img.shape[1] * 1.0 / coh.shape[1]),
                       order=1)
    seg = coh > threshold

    seg = segmentation_postprocessing(seg,
                                      kernel_size=15,
                                      stride=1,
                                      convex=convex)
    return seg
